...
simply X + II. The number 27 is written as XXVII, which is
XX + V + II.
...
#the fastest
def romanToInt(s):
    sExceptions = {"IV":4, "IX":9, "XL":40, "XC":90, "CD":400, "CM":900}
    sValues = {"I":1, "II":2, "III":3,  "V":5, "X":10, "L":50, "C":100, "D":500, "M":1000}

    s0  = s
    endnum = []
    for i in sExceptions.keys() :
        if i in s0 :
            endnum.append(sExceptions[i])
            s0 = s0.replace(i,"")
            #print (i, endnum, s0)
        else : continue

    for i in s0 :
        if i in sValues.keys() :
            endnum.append(sValues[i])
            s0 = s0.replace(i,"")
            #print (i, endnum, s0)
...
def andrisMethod(s):
    ...
    while i < len(s):
        if s[i:i+2] in sExceptions:
            total += sExceptions[s[i:i+2]]
            i += 2
        else:
            total += sValues[s[i]]
            i += 1
